...
impl<'a> TcpSocket<'a> {
    ...
    {
        log::info!("[en-accept] enter, calling listen");
        match self.io.with_mut(|s, _| s.listen(local_endpoint)) {
            Ok(()) => {}
            Err(tcp::ListenError::InvalidState) => {
                log::warn!("[en-accept] listen() InvalidState");
                return Err(AcceptError::InvalidState);
            }
            Err(tcp::ListenError::Unaddressable) => {
                log::warn!("[en-accept] listen() Unaddressable");
                return Err(AcceptError::InvalidPort);
            }
        }
        log::info!("[en-accept] listen ok, entering poll_fn");

        poll_fn(|cx| {
            let state = self.io.with_mut(|s, _| {
                let st = s.state();
                log::info!("[en-accept] poll: state={:?}", st);
                match st {
                    tcp::State::Listen | tcp::State::SynSent | tcp::State::SynReceived => {
                        s.register_send_waker(cx.waker());
                        log::info!("[en-accept] registered send waker, Pending");
                        None
                    }
                    _ => {
                        log::info!("[en-accept] state is {:?}, Ready!", st);
                        Some(Ok(()))
                    }
                }
            });
...
impl<'d> TcpIo<'d> {
    ...
    fn with_mut<R>(&self, f: impl FnOnce(&mut tcp::Socket, &mut Interface) -> R) -> R {
        self.stack.with_mut(|i| {
            ...
            let after = socket.state();
            if before != after {
                log::info!("[en-tcp] state changed: {:?} -> {:?}", before, after);
            }
            i.waker.wake();
            res
        })
    ...
    fn with<R>(&self, f: impl FnOnce(&tcp::Socket, &Interface) -> R) -> R {
        self.stack.with(|i| {
            ...
            if matches!(state, tcp::State::SynReceived | tcp::State::Established) {
                log::info!("[en-tcp] with(): state={:?}", state);
            }
            f(socket, &i.iface)
        })
    }
