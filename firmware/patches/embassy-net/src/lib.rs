...
impl<'d, D: Driver> Runner<'d, D> {
    ...
    pub async fn run(&mut self) -> ! {
        // FIX: the esp-radio WiFi driver never calls the registered waker
        // on packet arrival after the initial DHCP burst (hardware finding
        // on M5Stack C6L / ESP32-C6 / esp-radio 0.18). The original
        // poll_fn-based run() parks the task and it is never re-polled.
        //
        // This replacement uses a poll + timer loop: the real task waker
        // is still registered with the driver during each poll (so waker
        // events, if they ever fire, re-enqueue the task immediately),
        // and a 10ms timer guarantees re-polling regardless of waker
        // functionality. Latency cost: up to 10ms per packet.
        let mut poll_count: u64 = 0;
        loop {
            poll_count += 1;
            if poll_count <= 5 || poll_count % 100 == 0 {
                log::debug!("[en-runner] poll #{}", poll_count);
            }

            // Single poll iteration with the real task waker registered
            poll_fn(|cx| {
            ...
            // Re-poll after 10ms (timer-based fallback for broken wakers)
