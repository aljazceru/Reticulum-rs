...
/// Diagnostic counters — readable from the app (the modem task prints
/// them; direct console output from the fork has crate-instance issues).
#[unsafe(no_mangle)]
pub static RTOS_DIAG: [core::sync::atomic::AtomicU32; 8] =
    [const { core::sync::atomic::AtomicU32::new(0) }; 8];

/// Diagnostic counter increment (see RTOS_DIAG).
#[macro_export]
macro_rules! diag_count {
    ($idx:expr) => {
        crate::RTOS_DIAG[$idx].fetch_add(1, core::sync::atomic::Ordering::Relaxed);
    };
    ($idx:expr, $($arg:tt)*) => {
        crate::RTOS_DIAG[$idx].fetch_add(1, core::sync::atomic::Ordering::Relaxed);
    };
