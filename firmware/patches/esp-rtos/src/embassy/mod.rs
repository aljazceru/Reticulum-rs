...
/// Arguments passed to a thread that hosts an embassy [`Executor`].
#[cfg(feature = "esp-radio")]
struct ThreadExecutorArgs<F> {
    executor: *mut Executor,
    init: F,
}

// SAFETY: `executor` points at a `'static` executor exclusively owned by the
// spawned thread after `Executor::spawn` returns. `F` is `Send`.
#[cfg(feature = "esp-radio")]
unsafe impl<F: Send> Send for ThreadExecutorArgs<F> {}

/// Entry point for an RTOS thread that runs an embassy thread-mode executor.
#[cfg(feature = "esp-radio")]
extern "C" fn thread_executor_entry<F: FnOnce(Spawner)>(param: *mut c_void) {
    let args = unsafe { alloc::boxed::Box::from_raw(param.cast::<ThreadExecutorArgs<F>>()) };
    let executor = unsafe { &mut *args.executor };
    executor.run(args.init);
}
...
impl Executor {
    ...
    /// Spawn this executor on a new RTOS thread and return.
    ///
    /// The scheduler must already be running. `init` runs on the new thread and
    /// is followed by the executor poll loop, which never returns on that
    /// thread.
    ///
    /// `priority` is the RTOS priority of the new thread (`0` is the main
    /// thread). Keep this below the esp-radio driver threads so a busy
    /// `embassy-net` poll cannot starve the Wi-Fi blob.
    ///
    /// After this returns the caller must not use `self` again — the new thread
    /// exclusively owns the executor.
    #[cfg(feature = "esp-radio")]
    pub fn spawn<F>(
        &'static mut self,
        name: &'static str,
    ...
    {
        let args = alloc::boxed::Box::new(ThreadExecutorArgs {
            executor: self,
            init,
        });
        crate::SCHEDULER.create_task(
            name,
            thread_executor_entry::<F>,
            alloc::boxed::Box::into_raw(args).cast(),
            stack_size,
            priority as u32,
            None,
        );
    ...
    fn run_inner(
        ...
        loop {
            crate::diag_count!(6); // executor poll cycles
            hooks.before_poll();
            ...
            // If a task (typically embassy-net's Runner) woke itself during
            // poll, the wait below would return immediately and this thread
            // would busy-spin, starving other RTOS threads (Wi-Fi driver) of
            // the CPU. Yield first so the scheduler can run them.
            if flags.get() {
                crate::task::yield_task();
            }
            ...
            flags.wait();
            crate::diag_count!(7); // executor woke
        }
    }
}
