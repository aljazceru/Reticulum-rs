...
| 9.1 Hardware test ladder | ✅ `docs/hardware-testing.md`: unit → emulator (`rn node-sim`, faithful device-side protocol incl. frequency quantization + flow control + telemetry) → mock bridges → hardware-in-the-loop (`tests/hardware_rnode.rs` env-gated via `RETICULUM_HW_RNODE[_TCP]`, covers detect/firmware gate/identity, radio validation, TX counter) → over-the-air checklist; field-validated on a real Heltec V3 (officially supported hardware): the full serial command path — detect, firmware gate, config echo with real SX1262 quantization, telemetry, BT-enable byte + `CMD_FW_HASH` provisioning — runs against genuine firmware 1.86/1.80; the radio stays offline on this specific unit because `btStart()` fails (firmware `device_init()` gate; no BLE advertisement with the BT byte set, hash queries zero) and the reference Python fails identically — exact parity. Radio-level tier-4/5 awaits a unit whose BT controller initialises (e.g. factory-fresh via `rnodeconf --autoinstall`) |
